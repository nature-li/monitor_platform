...
$(document).on("click", ".machine-delete-button", function () {
    ...
    window.current_machine_id = $tr.find("td:eq(1)").text();
    ...
    BootstrapDialog.show({
        message: function (dialog) {
            ...
            content += '<div class="form-group">' +
                ...
                '<div class="col-xs-8">' +
                '<input id="ssh_user" type="text" class="form-control" value="' + ssh_user + '" disabled>' +
                '</div>' +
                '</div>';
            content += '<div class="form-group">' +
                '<label class="col-xs-2">SSH地址:</label>' +
                '<div class="col-xs-8">' +
                '<input id="ssh_ip" type="text" class="form-control" value="' + ssh_ip + '" disabled>' +
                '</div>' +
                '</div>';
            content += '<div class="form-group">' +
...
function edit_machine_page_view(response) {
    ...
    $('#machine_list_result > tbody  > tr').each(function () {
        var $check_box = $(this).find("td:eq(0)").find("input[name='machine_list[]']");
        var bind_machine_id = $check_box.val();
...
$("#add_machine_button").click(function () {
    BootstrapDialog.show({
        message: function (dialog) {
            ...
            // ssh_port
            content += '<div class="form-group"><input id="add_ssh_port" type="text" class="form-control" placeholder="输入SSH端口"></div>';
            // footer
            content += '</div>';
